...
class DocumentController {
    ...
    def save() {
        ...
        if (documentInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'document.label', default: 'Document'), documentInstance.id])}"
        ...
        if (documentInstance) {
            if (params.version) {
                ...
                if (documentInstance.version > version) {
                    documentInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'document.label', default: 'Document')] as Object[], "Another user has updated this Document while you were editing")
            ...
            if (!documentInstance.hasErrors() && documentInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'document.label', default: 'Document'), documentInstance.id])}"
    ...
    def delete() {
        ...
        if (documentInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'document.label', default: 'Document'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'document.label', default: 'Document'), params.id])}"
        ...
        if (!(file?.size || command.fileUri)) {
            ...
            if (documentInstance.validate() && !documentInstance.hasErrors()) {
                ...
                if (shipmentInstance) {
                    ...
                    flash.message = "${g.message(code: 'document.successfullySavedToShipment.message', args: [shipmentInstance?.name])}"
                } else if (orderInstance) {
                    orderInstance.addToDocuments(documentInstance).save(flush: true)
                    flash.message = "${g.message(code: 'document.successfullySavedToOrder.message', args: [orderInstance?.name])}"
                } else if (requestInstance) {
                    requestInstance.addToDocuments(documentInstance).save(flush: true)
                    flash.message = "${g.message(code: 'document.successfullySavedToRequest.message', args: [requestInstance?.description])}"
                } else if (productInstance) {
                    productInstance.addToDocuments(documentInstance).save(flush: true)
                    flash.message = "${g.message(code: 'document.succesfullyUpdatedDocument.message')}"
                } else if (invoiceInstance) {
                    invoiceInstance.addToDocuments(documentInstance).save(flush: true)
                    flash.message = "${g.message(code: 'document.successfullySavedToInvoice.message', args: [invoiceInstance?.name])}"
                }
            }
