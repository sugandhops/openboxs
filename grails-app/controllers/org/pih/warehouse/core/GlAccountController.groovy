...
class GlAccountController {
    ...
    def save() {
        ...
        if (glAccount.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'glAccount.label', default: 'GL Account'), glAccount.id])}"
    ...
    def update() {
        ...
        if (glAccount) {
            ...
            if (!glAccount.hasErrors() && glAccount.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'glAccount.label', default: 'GL Account'), glAccount.id])}"
                redirect(action: "list")
            } else {
                def glAccountType = glAccount?.glAccountType ? GlAccountType.get(glAccount?.glAccountType?.id) : null
            ...
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'glAccount.label', default: 'GL Account'), params.id])}"
            redirect(action: "list")
        }
    }
    ...
    def delete() {
        ...
        if (glAccount) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'glAccount.label', default: 'GL Account'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'glAccount.label', default: 'GL Account'), params.id])}"
