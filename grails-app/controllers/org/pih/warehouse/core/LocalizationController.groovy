...
class LocalizationController {
    ...
    def list() {
        ...
        println ' localizationInstanceList ::' +localizationInstanceList
    ...
    def save() {
        ...
        if (!localizationInstance) {
            ...
            if (params.version) {
                ...
                if (localizationInstance.version > version) {
                    ...
                    localizationInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'localization.label', default: 'Localization')] as Object[], "Another user has updated this Localization while you were editing")
        ...
        if (!localizationInstance.hasErrors() && localizationInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.saved.message', args: [g.message(code: 'localization.label', default: 'Localization'), localizationInstance.id])}"
    ...
    def show() {
        def localizationInstance = Localization.get(params.id)
        if (!localizationInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'localization.label', default: 'Localization'), params.id])}"
    ...
    def update() {
        ...
        if (localizationInstance) {
            ...
            localizationInstance.properties = params
            if (!localizationInstance.hasErrors() && localizationInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'localization.label', default: 'Localization'), localizationInstance.id])}"
                redirect(action: "list", id: localizationInstance.id)
            } else {
                render(view: "edit", model: [localizationInstance: localizationInstance])
            }
    ...
    def delete() {
        ...
        if (localizationInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'localization.label', default: 'Localization'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'localization.label', default: 'Localization'), params.id])}"
    ...
    def upload(LocalizationCommand command) {
        ...
        try {
            ...
            if (command.validate() && !command.hasErrors()) {
                ...
                flash.message = "${g.message(code: 'default.uploaded.message', args: [g.message(code: 'localizations.label')])}"
            } else {
