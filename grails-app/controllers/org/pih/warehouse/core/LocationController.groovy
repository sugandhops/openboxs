...
class LocationController {
    ...
    def edit() {
        ...
        if (!locationInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'location.label', default: 'Location'), params.id])}"
            redirect(action: "list")
        } else {
        ...
        if (locationInstance) {
            if (params.version) {
                ...
                if (locationInstance.version > version) {
                    ...
                    locationInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [
                            g.message(code: 'location.label', default: 'Location')] as Object[], "Another user has updated this Location while you were editing")
            ...
            if (locationInstance.validate() && !locationInstance.hasErrors()) {
                try {
                    ...
                    flash.message =
                        g.message(code: 'default.updated.message', args: [g.message(code: 'location.label', default: 'Location'), locationInstance.id])
        ...
        if (locationInstance) {
            try {
                if (locationInstance.isZoneLocation() && Location.findAllByZone(locationInstance)) {
                    flash.message = "${g.message(code: 'location.zoneAssigned.message')}"
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'location.label', default: 'Location'), params.id])}"
                ...
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'location.label', default: 'Location'), params.id])}"
                redirect(action: "edit", id: params.id)
            }
        } else {
            flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'location.label', default: 'Location'), params.id])}"
    ...
    def showContents() {
        ...
        if (!binLocation) {
            render "${g.message(code: 'default.not.found.message', args: [g.message(code: 'location.label', default: 'Location'), params.id])}"
            ...
            if (locationInstance) {
                ...
                if (!logo?.empty && logo.size < 1024 * 1000) { // not empty AND less than 1MB
                    ...
                    if (!locationInstance.hasErrors()) {
                        inventoryService.saveLocation(locationInstance)
                        flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'warehouse.label', default: 'Location'), locationInstance.id])}"
                    } else {
                        // there were errors, the logo was not saved
                        flash.message = "${g.message(code: 'default.not.updated.message', args: [g.message(code: 'user.label'), locationInstance.id])}"
                        render(view: "uploadPhoto", model: [locationInstance: locationInstance])
                        return
                    }
                } else {
                    flash.message = "${g.message(code: 'user.photoTooLarge.message', args: [g.message(code: 'location.label'), locationInstance.id])}"
                }

                redirect(action: "edit", id: locationInstance.id)
            } else {
                "${g.message(code: 'default.not.found.message', args: [g.message(code: 'warehouse.label', default: 'Location'), params.id])}"
            }
        }
