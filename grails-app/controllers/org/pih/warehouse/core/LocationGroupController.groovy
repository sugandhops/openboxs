...
class LocationGroupController {
    ...
    def save() {
        ...
        if (locationGroupInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'locationGroup.label', default: 'LocationGroup'), locationGroupInstance.id])}"
    ...
    def show() {
        ...
        if (!locationGroupInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'locationGroup.label', default: 'LocationGroup'), params.id])}"
        ...
        if (locationGroupInstance) {
            if (params.version) {
                ...
                if (locationGroupInstance.version > version) {
                    locationGroupInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'locationGroup.label', default: 'LocationGroup')] as Object[], "Another user has updated this LocationGroup while you were editing")
            ...
            if (!locationGroupInstance.hasErrors() && locationGroupInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'locationGroup.label', default: 'LocationGroup'), locationGroupInstance.id])}"
                redirect(action: "list", id: locationGroupInstance.id)
            } else {
                render(view: "edit", model: [locationGroupInstance: locationGroupInstance])
            }
    ...
    def delete() {
        ...
        if (locationGroupInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'locationGroup.label', default: 'LocationGroup'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'locationGroup.label', default: 'LocationGroup'), params.id])}"
