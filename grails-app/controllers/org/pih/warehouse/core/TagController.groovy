...
class TagController {
    ...
    def save() {
        ...
        if (tagInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'tag.label', default: 'Tag'), tagInstance.id])}"
    ...
    def show() {
        ...
        if (!tagInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'tag.label', default: 'Tag'), params.id])}"
        ...
        if (tagInstance) {
            if (params.version) {
                ...
                if (tagInstance.version > version) {
                    ...
                    tagInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'tag.label', default: 'Tag')] as Object[], "Another user has updated this Tag while you were editing")
            ...
            if (!tagInstance.hasErrors() && tagInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'tag.label', default: 'Tag'), tagInstance.id])}"
    ...
    def delete() {
        ...
        if (tagInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'tag.label', default: 'Tag'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'tag.label', default: 'Tag'), params.id])}"
