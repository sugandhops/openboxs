...
class UnitOfMeasureConversionController {
    ...
    def save() {
        ...
        if (!unitOfMeasureConversion.hasErrors() && unitOfMeasureConversionDataService.save(unitOfMeasureConversion)) {
            def messageArgs = [g.message(code: 'unitOfMeasureConversion.label', default: 'Unit of Measure conversion'), unitOfMeasureConversion.id]
            flash.message = "${g.message(code: 'default.created.message', args: messageArgs)}"
    ...
    def update() {
        UnitOfMeasureConversion unitOfMeasureConversion = unitOfMeasureConversionDataService.get(params.id)
        def messageArgs = [g.message(code: 'unitOfMeasureConversion.label', default: 'Unit of Measure conversion'), params.id]
        if (unitOfMeasureConversion) {
            unitOfMeasureConversion.properties = params
            if (!unitOfMeasureConversion.hasErrors() && unitOfMeasureConversionDataService.save(unitOfMeasureConversion)) {
                flash.message = "${g.message(code: 'default.updated.message', args: messageArgs)}"
                redirect(action: "list")
            } else {
                render(view: "edit", id: unitOfMeasureConversion.id, model: [unitOfMeasureConversion: unitOfMeasureConversion])
            }
        } else {
            flash.message = "${g.message(code: 'default.not.found.message', args: messageArgs)}"
            redirect(action: "list")
        }
    }
    ...
    def delete() {
        ...
        if (unitOfMeasureConversion) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: messageArgs)}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: messageArgs)}"
