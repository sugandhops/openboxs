...
class InventoryController {
    ...
    def save() {
        ...
        if (!warehouseInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [warehouse.message(code: 'warehouse.label', default: 'Location'), params.id])}"
            ...
            if (warehouseInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), warehouseInstance.inventory.id])}"
                redirect(action: "browse")
            } else {
    ...
    def show() {
        ...
        if (!inventoryInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), params.id])}"
            redirect(action: "list")
            return
        }
    ...
    def addToInventory() {
        ...
        if (!productInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'product.label', default: 'Product'), params?.product?.id])}"
            redirect(action: "browse")
        } else {
            def itemInstance = new InventoryItem(product: productInstance)
            if (!itemInstance.hasErrors() && itemInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), inventoryInstance.id])}"
                redirect(action: "browse", id: inventoryInstance.id)
            } else {
                flash.message = "${g.message(code: 'inventory.unableToCreateItem.message')}"
            }
        }
    }
        ...
        if (inventoryInstance) {
            if (params.version) {
                ...
                if (inventoryInstance.version > version) {
                    inventoryInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'inventory.label', default: 'Inventory')] as Object[],
    ...
    def delete() {
        ...
        if (inventoryInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), params.id])}"
                redirect(action: "list")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), params.id])}"
    ...
    def addItem() {
        ...
        if (itemInstance) {
            flash.message = "${g.message(code: 'default.alreadyExists.message', args: [g.message(code: 'inventory.label', default: 'Inventory item'), inventoryInstance.id])}"
            redirect(action: "show", id: inventoryInstance.id)
        } else {
            itemInstance = new InventoryItem(params)
            if (itemInstance.hasErrors() || !itemInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'inventory.label', default: 'Inventory item'), inventoryInstance.id])}"
                redirect(action: "show", id: inventoryInstance.id)
            } else {
                itemInstance.errors.each { println it }
                flash.message = "${g.message(code: 'default.notUpdated.message', args: [g.message(code: 'inventory.label', default: 'Inventory item'), inventoryInstance.id])}"
    ...
    def deleteItem() {
        ...
        if (itemInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), params.id])}"
                redirect(action: "show", id: params.inventory.id)
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), params.id])}"
                redirect(action: "show", id: params.inventory.id)
            }
        } else {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'inventory.label', default: 'Inventory'), params.id])}"
    ...
    def reorderReport() {
        ...
        def csv = ""
        csv += '"' + "${g.message(code: 'inventoryLevel.status.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.productCode.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'category.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.tags.label', default: 'Tags')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.unitOfMeasure.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.minQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.reorderQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.maxQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventory.averageMonthlyDemand.label', default: "Average Monthly Demand")}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventory.quantityAvailable.label', default: 'Quantity Available')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventory.quantityToOrder.label', default: 'Quantity to Order')}" + '"' + ","

        if (hasRoleFinance) {
            csv += '"' + "${g.message(code: 'product.unitCost.label', default: 'Unit Cost')}" + '"' + ","
            csv += '"' + "${g.message(code: 'inventory.expectedReorderCost.label', default: 'Expected Reorder Cost')}" + '"' + ","
        }
        ...
        inventoryItems.each { inventoryItem ->
            ...
            def statusMessage = "${g.message(code: 'enum.InventoryLevelStatusCsv.' + status)}"
    ...
    def deleteTransaction() {
        ...
        if (transactionInstance) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'transaction.label', default: 'Transaction'), params.id])}"
                redirect(action: "listTransactions")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'transaction.label', default: 'Transaction'), params.id])}"
                redirect(action: "editTransaction", id: params.id)
            }
        } else {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'transaction.label', default: 'Transaction'), params.id])}"
            redirect(action: "listTransactions")
        }
    }
    ...
    def saveTransaction() {
        ...
        if (saved) {
            flash.message = "${g.message(code: 'inventory.transactionSaved.message')}"
            redirect(action: "editTransaction", id: transactionInstance?.id)
        } else {
            flash.message = "${g.message(code: 'inventory.unableToSaveTransaction.message')}"
    ...
    def showTransaction() {
        def transactionInstance = Transaction.get(params.id)
        if (!transactionInstance) {
            flash.message = "${g.message(code: 'inventory.noTransactionWithId.message', args: [params.id])}"
            transactionInstance = new Transaction()
        }
    ...
    def confirmTransaction() {
        ...
        if (transactionInstance?.confirmed) {
            ...
            flash.message = "${g.message(code: 'inventory.transactionHasBeenUnconfirmed.message')}"
        } else {
            ...
            flash.message = "${g.message(code: 'inventory.transactionHasBeenConfirmed.message')}"
        }
        redirect(action: "listTransactions")
    }
    ...
    private def getCsvForInventoryMap(map, statusMap) {
        ...
        csv += '"' + "${g.message(code: 'category.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.tags.label', default: 'Tags')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.binLocation.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.unitOfMeasure.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.minQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.reorderQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.maxQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.forecastQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.currentQuantity.label', default: 'Current quantity')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.pricePerUnit.label')}" + '"' + ","
        ...
        map.each { inventoryItem, quantity ->
            ...
            if (!statusMessage) {
                ...
                statusMessage = "${g.message(code: 'enum.InventoryLevelStatusCsv.' + status)}"
            }
    ...
    private def getCsvForProductMap(inventoryItems) {
        ...
        csv += '"' + "${g.message(code: 'category.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.tags.label', default: 'Tags')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.abcClass.label', default: 'ABC Class')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.unitOfMeasure.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.minQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.reorderQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.maxQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.forecastQuantity.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'inventoryLevel.currentQuantity.label', default: 'Current quantity')}" + '"' + ","
        csv += '"' + "${g.message(code: 'default.quantityAvailableToPromise.label', default: 'Quantity ATP')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.pricePerUnit.label')}" + '"' + ","
        csv += '"' + "${g.message(code: 'product.totalValue.label')}" + '"'
        csv += "\n"
