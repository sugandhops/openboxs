...
class InventoryItemController {
    ...
    def showCurrentStock(StockCardCommand cmd) {
        ...
        commandInstance.product = Product.get(params?.id)
         println 'commandInstance.product :: '+commandInstance.product
    ...
    def showLotNumbers(StockCardCommand cmd) {
        ...
        // now populate the rest of the commmand object
        def commandInstance = inventoryService.getStockCardCommand(cmd, params)
        //println ' commandInstance :: ' + commandInstance
    ...
    def saveInventoryItem() {
        ...
        if (!inventoryItem.hasErrors() && inventoryItem.save()) {
            ...
            flash.message = "${g.message(code: 'inventoryItem.savedItemWithinNewTransaction.message', args: [inventoryItem.id, transactionInstance.id])}"
    ...
    def edit() {
        ...
        if (!itemInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), params?.id])}"
    ...
    def updateInventoryLevel() {
        ...
        if (!inventoryLevelInstance.hasErrors() && inventoryLevelInstance.save()) {
            ...
            flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'inventoryLevel.label', default: 'Inventory level')])}"
        } else {
    ...
    def adjustStock(AdjustStockCommand command) {
        ...
        try {
            ...
            if (!command.hasErrors()) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), inventoryItem.id])}"
            }
        ...
        if (itemInstance) {
            if (params.version) {
                ...
                if (itemInstance.version > version) {
                    itemInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'inventoryItem.label', default: 'Inventory Item')] as Object[], "Another user has updated this inventory item while you were editing")
            ...
            if(itemInstance.product && itemInstance.product.lotAndExpiryControl && (!params.expirationDate || !params.lotNumber)) {
                flash.error = "${g.message(code: 'inventoryItem.lotAndExpiryControl.message')}"
                redirect(controller: "inventoryItem", action: "showStockCard", id: productInstance?.id)
                return
            }
            ...
            if (!itemInstance.product.lotAndExpiryControl && !itemInstance.lotNumber) {
                flash.error = "${g.message(code: 'inventoryItem.blankLot.message')}"
                redirect(controller: "inventoryItem", action: "showStockCard", id: productInstance?.id)
                return
            }

            if (!itemInstance.hasErrors() && inventoryItemDataService.save(itemInstance)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), itemInstance.id])}"
            } else {
                flash.error = "${g.message(code: 'default.not.updated.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), itemInstance.id])}"
    ...
    def addToShipment() {
        ...
        try {
            ...
            if (shipmentItem.hasErrors() || !shipmentItem.validate()) {
                flash.message = "${g.message(code: 'inventoryItem.errorValidatingItem.message')}\n"
            ...
            if (!shipmentItem.hasErrors()) {
                if (!shipmentInstance.addToShipmentItems(shipmentItem).save()) {
                    ...
                    flash.message = "${g.message(code: 'inventoryItem.unableToAddItemToShipment.message')}"
                } else {
                    def productDescription = format.product(product: productInstance).decodeHTML() + (inventoryItem?.lotNumber) ? " #" + inventoryItem?.lotNumber : ""
                    flash.message = "${g.message(code: 'inventoryItem.addedItemToShipment.message', args: [productDescription, shipmentInstance?.name])}"
                }
            }
    ...
    def saveInventoryLevel() {
        ...
        if (!inventoryLevelInstance.hasErrors() && inventoryLevelInstance.save()) {
            ...
            flash.message = "${g.message(code: 'inventoryItem.errorSavingInventoryLevels.message')}<br/>"
    ...
    def create() {
        ...
        if (InventoryItem && inventoryItem.save()) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'inventoryItem.label'), params?.id])}"
        } else {
            flash.message = "${g.message(code: 'default.not.created.message', args: [g.message(code: 'inventoryItem.label')])}"
        }
    ...
    def delete() {
        ...
        if (inventoryItem) {
            try {
                ...
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'inventoryItem.label', default: 'Attribute'), params.id])}"
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'inventoryItem.label', default: 'Attribute'), params.id])}"
            }
        } else {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'inventoryItem.label', default: 'Attribute'), params.id])}"
        }
    ...
    def saveTransactionEntry() {
        def productInstance = Product.get(params?.product?.id)
        if (!productInstance) {
            flash.message = "${g.message(code: 'default.notfound.message', args: [g.message(code: 'product.label', default: 'Product'), productInstance.id])}"
            redirect(action: "showStockCard", id: productInstance?.id)
        } else {
            def inventoryItem = inventoryService.findByProductAndLotNumber(productInstance, params.lotNumber ?: null)
            if (!inventoryItem) {
                flash.message = "${g.message(code: 'default.notfound.message', args: [g.message(code: 'inventoryItem.label', default: 'Inventory item'), params.lotNumber])}"
            } else {
                def transactionInstance = new Transaction(params)
                def transactionEntry = new TransactionEntry(params)
                ...
                if (!transactionEntry.hasErrors() &&
                    ...
                    flash.message = "${g.message(code: 'inventoryItem.savedTransactionEntry.label')}"
                } else {
                    transactionInstance.errors.each { log.info "${it}" }
                    transactionEntry.errors.each { log.info "${it}" }
                    flash.message = "${g.message(code: 'inventoryItem.inventoryItem.unableToSaveTransactionEntry.message.label')}"
                }
            }
        }
        redirect(action: "showStockCard", params: ['product.id': productInstance?.id])
    ...
    def recall() {
        ...
        if (userService.isUserAdmin(session.user)) {
            if (inventoryItem.lotNumber) {
                ...
                flash.message = "${g.message(code: 'inventoryItem.recall.message')}"
            } else {
                flash.message = "${g.message(code: 'inventoryItem.recallError.message')}"
            }
        } else {
            flash.message = "${g.message(code: 'errors.noPermissions.label')}"
        }
        redirect(action: 'showLotNumbers', params: ['product.id': inventoryItem?.product?.id])
    }
    ...
    def revertRecall() {
        ...
        if (userService.isUserAdmin(session.user)) {
            ...
            productAvailabilityService.refreshProductsAvailability(null, [inventoryItem?.product?.id], false)
            flash.message = "${g.message(code: 'inventoryItem.revertRecall.message')}"
        } else {
            flash.message = "${g.message(code: 'errors.noPermissions.label')}"
        }
        redirect(action: 'showLotNumbers', params: ['product.id': inventoryItem?.product?.id])
