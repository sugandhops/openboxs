...
class OrderController {
    ...
    def save() {
        ...
        if (orderInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'order.label', default: 'Order'), orderInstance.id])}"
    ...
    def placeOrder() {
        ...
        if (orderInstance) {
            if (orderInstance.hasErrors()) {
                ...
                flash.message = "${g.message(code: 'order.orderHasBeenPlacedWithVendor.message', args: [orderInstance?.orderNumber, orderInstance?.origin?.name])}"
                redirect(action: 'show', id: orderInstance.id)
            }
        } else {
            flash.message = "${g.message(code: 'order.notFound.message', args: [params.id], default: 'Order {0} was not found.')}"
            redirect(action: "list")
        }
    }
        ...
        if (orderInstance) {
            if (params.version) {
                ...
                if (orderInstance.version > version) {
                    ...
                    orderInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'order.label', default: 'Order')] as Object[], "Another user has updated this Order while you were editing")
            ...
            if (!orderInstance.hasErrors() && orderInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'order.label', default: 'Order'), orderInstance.id])}"
                redirect(action: "list", id: orderInstance.id, params: [orderType: orderInstance.orderType])
            } else {
                render(view: "edit", model: [orderInstance: orderInstance])
            }
    ...
    def remove() {
        ...
        if (orderInstance) {
            if (orderInstance.hasPrepaymentInvoice) {
                flash.message = "${g.message(code: 'order.errors.deletePrepaid.message')}"
                redirect(action: "show", id: orderInstance?.id)
                return
            }
            ...
            if (orderInstance.status == OrderStatus.PENDING) {
                try {
                    ...
                    flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'order.label', default: 'Order'), orderInstance.orderNumber])}"
                } catch (org.springframework.dao.DataIntegrityViolationException e) {
                    flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'order.label', default: 'Order'), orderInstance.orderNumber])}"
                }
            } else {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'order.label', default: 'Order'), orderInstance.orderNumber])}"
            }
    ...
    def editAdjustment() {
        ...
        if (!orderInstance) {
            ...
            if (!orderAdjustment) {
                ...
                flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'comment.label', default: 'Comment'), commentInstance.id])}"
                redirect(action: "show", id: orderInstance?.id)
            }
        ...
        if (orderInstance) {
            if (currentLocation.isAccountingRequired()) {
                ...
                if (!orderAdjustmentType.glAccount) {
                    render(status: 500, text: "${g.message(code: 'orderAdjustment.missingGlAccount.label')}")
                    return
                }
            }
            def orderAdjustment = OrderAdjustment.get(params?.id)
            ...
            if (orderAdjustment) {
                ...
                if (orderAdjustment.save(flush: true)) {
                    flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'orderAdjustment.label', default: 'Order Adjustment'), orderAdjustment.id])}"
    ...
    def deleteAdjustment() {
        ...
        if (!orderAdjustment) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'orderAdjustment.label', default: 'Order Adjustment'), params.id])}"
            redirect(action: "show", id: params.order.id)
        }

        orderService.deleteAdjustment(orderAdjustment, user)

        flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'order.label', default: 'Order'), params.order.id])}"
    ...
    def deleteComment() {
        def orderInstance = Order.get(params.order.id)
        if (!orderInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'order.label', default: 'Order'), params.order.id])}"
            redirect(action: "list")
        } else {
            def commentInstance = Comment.get(params?.id)
            if (!commentInstance) {
        ...
        if (orderInstance) {
            ...
            if (commentInstance) {
                ...
                if (!commentInstance.hasErrors() && commentInstance.save(flush: true)) {
                    flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'comment.label', default: 'Comment'), commentInstance.id])}"
                    redirect(action: "show", id: orderInstance.id)
                } else {
                    render(view: "addComment", model: [orderInstance: orderInstance, commentInstance: commentInstance])
                }
    ...
    def addOrderItemToShipment() {
        ...
        if (orderItem) {
            ...
            if (!shipmentInstance.hasErrors() && shipmentInstance?.save(flush: true)) {
                ...
                flash.message = "${g.message(code: 'order.shipmentItemErrors.message')}"
    ...
    def download() {
        ...
        if (!orderInstance) {
            ...
            csv.printRecord(
                ...
                g.message(code: 'product.supplierCode.label'),
                g.message(code: 'product.manufacturerCode.label'),
                g.message(code: 'orderItem.quantity.label'),
                g.message(code: 'product.unitOfMeasure.label'),
                g.message(code: 'orderItem.unitPrice.label'),
                g.message(code: 'orderItem.totalPrice.label'),
                g.message(code: 'orderItem.budgetCode.label')
            )
    ...
    def saveOrderItem() {
        ...
        if (validationCode == ValidationCode.BLOCK) {
            render(status: 500, text: "${g.message(code: 'orderItem.blockedSupplier.label')}")
            return
        }
        ...
        if (currentLocation.isAccountingRequired()) {
            ...
            if (!product.glAccount) {
                render(status: 500, text: "${g.message(code: 'orderItem.missingGlAccount.label')}")
                return
            }
        }
        if (!orderItem) {
    ...
    def downloadOrderItems() {
        ...
        if (!orderInstance) {
            ...
            def csv = CSVUtils.getCSVPrinter()
            csv.printRecord(
                    g.message(code: 'orderItem.id.label'),
                    g.message(code: 'product.productCode.label'),
                    g.message(code: 'product.name.label'),
                    g.message(code: 'product.sourceCode.label'),
                    g.message(code: 'product.sourceName.label'),
                    g.message(code: 'product.supplierCode.label'),
                    g.message(code: 'product.manufacturer.label'),
                    g.message(code: 'product.manufacturerCode.label'),
                    g.message(code: 'default.quantity.label'),
                    g.message(code: 'default.unitOfMeasure.label'),
                    g.message(code: 'default.cost.label'),
                    g.message(code: 'orderItem.totalCost.label'),
                    g.message(code: 'order.recipient.label'),
                    g.message(code: 'orderItem.estimatedReadyDate.label'),
                    g.message(code: 'orderItem.actualReadyDate.label'),
                    g.message(code: 'orderItem.budgetCode.label')
            )
    ...
    def createCombinedShipment() {
        ...
        if (!(orderInstance.destination.equals(currentLocation) || currentLocation.supports(ActivityCode.ENABLE_CENTRAL_PURCHASING))) {
            flash.message = "${g.message(code:'order.cantShipFromDifferentLocation.label')}"
        ...
        if (!orderInstance.orderItems.find {it.quantityRemainingToShip != 0 && it.orderItemStatusCode != OrderItemStatusCode.CANCELED }) {
            flash.message = "${g.message(code:'purchaseOrder.noItemsToShip.label')}"
