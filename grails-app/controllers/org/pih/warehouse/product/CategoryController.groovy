...
class CategoryController {
    ...
    def move() {
        ...
        if (!child.hasErrors()) {
            ...
            flash.message = "${g.message(code: 'default.success.message')}"
        }
    ...
    def saveCategory() {
        ...
        try {
            ...
            flash.message = "${g.message(code: 'category.saved.message', arg: [format.category(category: categoryInstance).decodeHTML()])}"
    ...
    def deleteCategory() {
        ...
        if (categoryInstance) {
            try {
                ...
                flash.error = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'category.label', default: 'Category'), params.id])}"
                List<Category> categoriesWithoutParent = productService.getCategoriesWithoutParent()
    ...
    def create() {
        ...
        categoryInstance.properties = params
        println "categoryInstance.properties: "+ categoryInstance.properties
        ...
        if (categoryInstance) {
            if (params.version) {
                ...
                if (categoryInstance.version > version) {
                    ...
                    categoryInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'category.label', default: 'Category')] as Object[], "Another user has updated this Category while you were editing")
                    render(view: "edit", model: [categoryInstance: categoryInstance])
                    return
                }
            }
    ...
    def delete() {
        ...
        if (categoryInstance) {
            try {
                categoryGormService.delete(params.id)
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'category.label', default: 'Category'), params.id])}"
                redirect(action: "tree")
            }
            catch (org.springframework.dao.DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'category.label', default: 'Category'), params.id])}"
