...
class ProductController { 
    ...
    def batchSave(BatchEditCommand cmd) {
        ...
        if (!cmd.hasErrors()) {
            flash.message = "${g.message(code: 'product.allSavedSuccessfully.message')}"
    ...
    def list() {
       ...
       productInstanceList = productService.getAllProducts()
    ...
    def create() {
        println "productInstance params: "+params
        def startTime = System.currentTimeMillis()
    ...
    def save() {
        ...
        if (!productInstance.hasErrors() && productService.saveProduct(productInstance)) {
            ...
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'product.label', default: 'Product').decodeHTML()])}"
    ...
    def renderTemplate() {
        ...
        if (!productInstance && renderNotFoundError) {
            def text = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'product.label', default: 'Product'), params.id])}"
            render(text: text)
        } else {
    ...
    def update() {
        ...
        if (productInstance) {
            if (params.version) {
                ...
                if (productInstance.version > version) {
                    productInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [
                            g.message(code: 'product.label', default: 'Product')] as Object[], "Another user has updated this product while you were editing")
            ...
            try {
                ...
                productInstance.validateRequiredFieldsInLocation(location)
                println 'Test :: '+productInstance
                 
                if (!productInstance.hasErrors() && productInstance.save(failOnError: true, flush: true)) {
                    //flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'product.label', default: 'Product').decodeHTML(), format.product(product: productInstance).decodeHTML()])}"
    ...
    def savePackage() {
        ...
        if (!productInstance.hasErrors() && packageInstance.validate() && productInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'package.label', default: 'Product'), packageInstance.name])}"
            redirect(action: "edit", id: productInstance?.id)
        } else {
            def location = Location.get(session.warehouse.id)
            def inventoryLevelInstance = InventoryLevel.findByProductAndInventory(productInstance, location.inventory)
        ...
        if (params.url) {
            ...
            try {
                ...
                if (documentInstance?.validate() && !documentInstance?.hasErrors()) {
                    ...
                    flash.message = "${g.message(code: 'document.successfullySavedToProduct.message', args: [command?.product?.name])}"
                }
                // If there are errors, we need to redisplay the document form
                else {
                    log.info "Document did not save " + documentInstance.errors
                    flash.message = "${g.message(code: 'document.cannotSave.message', args: [documentInstance.errors])}"
            ...
            if (!file || file?.isEmpty()) {
                flash.message = "${g.message(code: 'document.documentCannotBeEmpty.message')}"
                ...
                if (!command?.product) {
                    ...
                    flash.message = "${g.message(code: 'document.productDoesNotExist.message')}"
                    redirect(controller: "product", action: "list")
                    return
                } else {
                ...
                flash.message = "${g.message(code: 'document.documentTooLarge.message')}"
            }
        }
    ...
    def deleteDocument() {
        ...
        if (!productInstance) {
            ...
            if (!documentInstance) {
                flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'document.label', default: 'Document'), params.id])}"
                redirect(action: "edit", id: productInstance?.id)
            } else {
    ...
    def uploadCsv(ImportDataCommand command) {
        ...
        if (request.method == "POST") {
            ...
            if (uploadFile && !uploadFile?.empty) {
                ...
                command.errors.reject("${g.message(code: 'import.emptyFile.message', default: 'File is empty')}")
            }
        }
    ...
    private def sendProductCreatedNotification(Product productInstance) {
        try {
            ...
            if (recipientList) {
                def subject = "${g.message(code: 'email.productCreated.message', args: [productInstance?.name, productInstance?.createdBy?.name])}"
    ...
    def merge() {
        ...
        flash.message = "${g.message(code: 'product.mergeProducts.success.message', args: [params.primaryProduct, params.obsoleteProduct])}"
