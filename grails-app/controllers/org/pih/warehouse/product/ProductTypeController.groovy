...
class ProductTypeController {
    ...
    def save() {
        ...
        if (productTypeService.saveProductType(productTypeInstance)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'productType.label', default: 'ProductType'), productTypeInstance.id])}"
    ...
    def show() {
        ...
        if (!productTypeInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'productType.label', default: 'ProductType'), params.id])}"
        ...
        if (productTypeInstance) {
            if (params.version) {
                ...
                if (productTypeInstance.version > version) {
                    ...
                    productTypeInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'productType.label', default: 'ProductType')] as Object[], "Another user has updated this ProductType while you were editing")
            ...
            if (productTypeService.saveProductType(productTypeInstance)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'productType.label', default: 'ProductType'), productTypeInstance.id])}"
                redirect(action: "list", id: productTypeInstance.id)
            }
            else {
                render(view: "edit", model: [productTypeInstance: productTypeInstance])
    ...
    def delete() {
        if (params.id == Holders.config.openboxes.identifier.defaultProductType.id) {
            flash.message = "${g.message(code: 'productType.cannotDeleteDefaultProductType.message')}"
        ...
        if (productTypeInstance) {
            ...
            if (existingProducts) {
                flash.message = "${g.message(code: 'productType.deleteWithExistingProducts.message')}"
                redirect(action: "list", id: params.id)
            }

            try {
                productTypeService.delete(productTypeInstance)
                flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'productType.label', default: 'ProductType'), params.id])}"
                redirect(action: "list")
            }
            catch (DataIntegrityViolationException e) {
                flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'productType.label', default: 'ProductType'), params.id])}"
                redirect(action: "list", id: params.id)
            }
        }
        else {
