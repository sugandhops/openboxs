    ...
    def createShipmentFlow() {
        ...
        saveLocationAction {
            action {
                ...
                if (locations) {
                    flash.message = "${g.message(code: 'location.alreadyExists.message', args: [locationInstance.name])}"
                    invalid()
                } else {
                    ...
                    if (locationInstance.save(flush: true) && !locationInstance.hasErrors()) {
                        ...
                        flash.message = "${g.message(code: 'location.created.message', args: [locationInstance.name])}"
                        valid()
                    } else {
                        log.info "invalid location " + locationInstance.errors
                        flash.message = "${g.message(code: 'location.invalid.message', args: [locationInstance.name])}"
                        flash.addLocation = true
        ...
        savePersonAction {
            action {
                ...
                if (persons) {
                    flash.message = "${g.message(code: 'person.alreadyExists.message', args: [personInstance.firstName, personInstance.lastName])}"
                    invalid()
                } else {
                    log.info "validate person"
                    if (!personInstance.validate()) {
                        log.info "invalid person " + personInstance.errors
                        flash.message = "${g.message(code: 'person.invalid.message', args: [personInstance.firstName, personInstance.lastName])}"
                        invalid()
                    } else {

                        if (personInstance.save(flush: true) && !personInstance.hasErrors()) {
                            log.info "saved person " + personInstance + " with id " + personInstance?.id
                            flash.message = "${g.message(code: 'person.created.message', args: [personInstance.firstName, personInstance.lastName])}"
                            valid()
                        } else {
                            log.info "invalid person " + personInstance.errors
                            flash.message = "${g.message(code: 'person.invalid.message', args: [personInstance.firstName, personInstance.lastName])}"
        ...
        saveShipperAction {
            action {
                ...
                if (shippers) {
                    flash.message = "${g.message(code: 'shipper.alreadyExists.message', args: [shipperInstance?.name])}"
                    invalid()
                } else {
                    log.info "validate shipper"
                    if (!shipperInstance.validate()) {
                        log.info "invalid person " + shipperInstance.errors
                        flash.message = "${g.message(code: 'shipper.invalid.message', args: [shipperInstance?.name])}"
                        invalid()
                    } else {
                        ...
                        if (shipperInstance.save(flush: true) && !shipperInstance.hasErrors()) {
                            ...
                            flash.message = "${g.message(code: 'shipper.created.message', args: [shipperInstance?.name])}"
                            valid()
                        } else {
        ...
        moveContainerAction {
            action {
                ...
                try {
                    ...
                    flash.message = "${g.message(code: 'default.unsupportedOperation.message')}"
                    error()
                }
                log.info "Old shipment " + oldShipment.id
            }
