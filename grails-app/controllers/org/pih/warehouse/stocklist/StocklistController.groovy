...
class StocklistController {
    ...
    def sendMail() {
        ...
        if (!params.recipients || !params.id || !params.body || !params.subject) {
            flash.error = "${g.message(code: 'email.noParams.message')}"
            ...
            flash.error = "${g.message(code: 'stockList.noManagerSelected.label')}"
            redirect(controller: "requisitionTemplate", action: "sendMail", params: [id: params.id])
        } else {
            ...
            if (Holders.config.grails.mail.enabled) {
                ...
                flash.message = "${g.message(code: 'email.disabled.message')}"
            }
