...
class AuthController {
    ...
    def handleLogin() {
        ...
        if (userInstance) {
            ...
            if (!userInstance?.active) {
                flash.message = "${g.message(code: 'auth.accountRequestUnderReview.message')}"
                redirect(controller: 'auth', action: 'login')
                return
            }
            ...
            if (userService.authenticate(params.username, params.password)) {
                ...
                //println 'userInstance :: ' + userInstance.warehouse

                session.user = userInstance
                session.userName = userInstance?.username
                //session.location.id = '1'
                session.warehouse = [id: 1]
                ...
                // Invalid password
                flash.message = "${g.message(code: 'auth.incorrectPassword.label', args: [params.username])}"
                userInstance = new User(username: params['username'])
                userInstance.errors.rejectValue("version", "default.authentication.failure",
                        [g.message(code: 'user.label', default: 'User')] as Object[], "${g.message(code: 'auth.unableToAuthenticateUser.message')}")
                render(view: "login", model: [userInstance: userInstance])
            }
        } else {
            flash.message = "${g.message(code: 'auth.userNotFound.message', args: [params.username])}"
    ...
    def logout() {
        if (session.impersonateUserId) {
            ...
            flash.message = "${g.message(code: 'auth.logoutSuccess.message')}"
            session.invalidate()
            redirect(action: 'login')
        }
    }
     ...
     */
    def signup() {
       Boolean enabled = grailsApplication.config.signup.enabled?:false
        if (!enabled) {
     ...
     */
     //@Transactional
    def handleSignup() {
        println grailsApplication.config.appName
        println grailsApplication.config.signup.enabled
        ...
        if ("POST".equalsIgnoreCase(request.getMethod())) {
            ...
            if (!userInstance.hasErrors() && userInstance.save(flush: true)) {
                ...
                flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'user.label'), userInstance.email])}"
                redirect(action: "login")
            } else {
