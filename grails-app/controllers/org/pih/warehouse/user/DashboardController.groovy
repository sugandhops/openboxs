...
class DashboardController {
    ...
    def index() {
        //println 'session.warehouse :: '+ session?.user

     //    if(userAgentIdentService != null){
    ...
    def status() {
        ...
        def results = comments.collect {
            if (it.dateSent > new Date()) {
                [id: it.id, comment: g.message(code: it.comment, args: [format.datetime(obj: it.dateSent)]), dateSent: it.dateSent]
            }
        }
        render results as JSON
    }
    ...
    def megamenu() {
        ...
        //println 'menuConfig asS' + menuConfig
        //println 'User :: '+user
        ...
        //println 'translatedMenu :: '+translatedMenu
        ...
        //println 'configurationSection asS :: ' + configurationSection 
        [
                menu                    : translatedMenu,
    ...
    def changeLocation() {
        ...
        Map loginLocationsMap = locationService.getLoginLocationsMap(user, user?.warehouse, true)
        List savedLocations = [user?.warehouse, session?.warehouse].unique() - null
        println 'savedLocations :: ' + user?.warehouse
    ...
    def downloadFastMoversAsCsv() {
        ...
        if (data?.results) {
            ...
            sw.append("${g.message(code: 'fastMovers.empty.message')}")
        }
