...
class UserController {
    ...
    def sendTestEmail = {
        def userInstance = User.get(params.id)
        if (!userInstance) {
            flash.message = "${g.message(code: 'default.not.found.message', args: [g.message(code: 'user.label'), params.id])}"
    ...
    def create = {
        ...
        // def hql = "FROM cos"
        // def query = Book.createQuery(hql)
    ...
    def save = {
        ...
        if (userInstance.save(flush: true)) {
            flash.message = "${g.message(code: 'default.created.message', args: [g.message(code: 'user.label'), userInstance.id])}"
            redirect(action: "edit", id: userInstance.id)
        } else {       
    ...
    def edit = {        
        ...
        if (!userInstance) {
            ...
            def roles = org.pih.warehouse.core.Role.list()
            
            roles = roles ? roles.sort { it.description } : []
            println 'roles ::' +roles
            // Pass roles to the view
            [model: [roles: roles, userInstance: userInstance]]   

            def locations = locationService.getLoginLocations(session.warehouse).sort()
            return [userInstance: userInstance, locations: locations, roles: roles]
        }
    }
    ...
    def toggleActivation = {
        ...
        if (!userInstance) {
            ...
            if (!userInstance.hasErrors() && userInstance.save(flush: true)) {
                flash.message = "${g.message(code: 'default.updated.message', args: [g.message(code: 'user.label'), userInstance.id])}"
   ...
   // @CacheFlush(["megamenuCache"])
        ...
        if (userInstance) {
            if (params.version) {
                ...
                if (userInstance.version > version) {
                    userInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [g.message(code: 'user.label')] as Object[], "Another user has updated this User while you were editing")
                    def locations = Location.AllDepotWardAndPharmacy()
                    render(view: "edit", model: [userInstance: userInstance, locations: locations])
                    return
                }
            ...
            try {
                userInstance = userService.updateUser(params.id, params.id, params)
                // Update session data if the user is editing their own profile
                if (params.id == userInstance?.id) {
    ...
    def delete = {
        ...
        if (userInstance) {
            if (userInstance?.id == session?.user?.id) {
                flash.message = "${g.message(code: 'default.cannot.delete.self.message', args: [g.message(code: 'user.label'), params.id])}"
                redirect(action: "edit", id: params.id)
            } else {
                try {
                    userInstance.delete(flush: true)
                    flash.message = "${g.message(code: 'default.deleted.message', args: [g.message(code: 'user.label'), params.id])}"
                    redirect(action: "list")
                }
                catch (org.springframework.dao.DataIntegrityViolationException e) {
                    flash.message = "${g.message(code: 'default.not.deleted.message', args: [g.message(code: 'user.label'), params.id])}"
                    redirect(action: "edit", id: params.id)
                }
            }
        } else {
    ...
    def viewPhoto = {
        ...
        if (userInstance) {
            ...
            "${g.message(code: 'default.not.found.message', args: [g.message(code: 'user.label'), params.id])}"
        }
    }
        ...
        if (userInstance) {
            ...
            if (!photo?.empty && photo.size < 1024 * 1000) { // not empty AND less than 1MB
                ...
                if (!userInstance.hasErrors() && userInstance.save(flush: true)) {
                    ...
                    flash.message = "${g.message(code: 'default.not.updated.message', args: [g.message(code: 'user.label'), userInstance.id])}"
                    render(view: "uploadPhoto", model: [userInstance: userInstance])
                    return
                }
            } else {
                flash.message = "${g.message(code: 'user.photoTooLarge.message', args: [g.message(code: 'user.label'), userInstance.id])}"
    ...
    def sendUserStatusChanged(User userInstance) {
        try {
            ...
            flash.message = "${g.message(code: 'email.sent.message')}"
        }
        catch (Exception e) {
            flash.message = "${g.message(code: 'email.notSent.message', args: [userInstance.email])}: ${e.message}"
        }
    ...
    def sendUserConfirmed(User userInstance) {
        ...
        try {
            sendMail {
                ...
                subject "${g.message(code: 'email.userConfirmed.message', args: [userInstance.username])}"
                html "${g.render(template: "/email/userConfirmed", model: [userInstance: userInstance])}"
            }
            flash.message = "${g.message(code: 'email.sent.message')}: ${e.message}"
