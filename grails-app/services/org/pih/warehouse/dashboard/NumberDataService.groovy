...
class NumberDataService {
    ...
    NumberData getInventoryByLotAndBin(def location) {
        ...
        return new NumberData(binLocations[0], "/openboxes/report/showBinLocationReport?location.id=1&status=inStock")
    }
    ...
    NumberData getInProgressShipments(def user, def location) {
        ...
        return new NumberData(
                requisitionShipmentCount + returnOrderShipmentCount,
                "/openboxes/stockMovement/list?direction=OUTBOUND&receiptStatusCode=PENDING&origin=1&createdBy=" + user.id
        )
    }
    ...
    NumberData getDefaultBin(def location) {
        ...
        return new NumberData(productsInDefaultBin[0], "/openboxes/report/showBinLocationReport?location.id=" + location + "&status=inStock")
    }
