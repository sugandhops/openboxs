...
class NotificationService {
    ...
    def sendUserAccountCreation(User userInstance, Map additionalQuestions) {
        try {
            ...
            println "recipients: "+userInstance?.email
            // if (recipients) {
                def locale = new Locale(grailsApplication.config.openboxes.locale.defaultLocale)
                def to = userInstance?.email//recipients?.collect { it.email }?.unique()
    ...
    def sendUserAccountConfirmation(User userInstance, Map additionalQuestions) {
        try {
            ...
            if (userInstance?.email) {
                // println 'Test---1'+userInstance
                // println 'Test---2'+grailsApplication.config.openboxes
                // println 'Test---3'+grailsApplication.config.openboxes.locale
                // println 'Test---4'+grailsApplication.config.openboxes.locale.defaultLocale
