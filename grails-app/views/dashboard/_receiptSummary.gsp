...
<div class="box">
    <h2><g:message code="receiving.label"/></h2>
	    			...
	    			<thead>
                        <tr class="prop odd">
                            <td colspan="3" class="left">
                                ...
                                <div class="fade">${g.message(code: 'dashboard.incoming.label', args: [session.warehouse.name, startDate, endDate]) }</div>
                            </td>
                        </tr>
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'incoming','status':pending]">
									${g.message(code: 'dashboard.incoming.pending.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
							<td class="right">
								<g:link controller="shipment" action="list" params="['type':'incoming','status':pending]">
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'incoming','status':shipped]">
									${g.message(code: 'dashboard.incoming.shipped.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
							<td class="right">
								<g:link controller="shipment" action="list" params="['type':'incoming','status':shipped]">
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'incoming','status':received]">
									${g.message(code: 'dashboard.incoming.received.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
							<td class="right">
								<g:link controller="shipment" action="list" params="['type':'incoming','status':received]">
