...
<div class="box">
    <h2><g:message code="shipping.label"/></h2>
                    ...
                    <thead>
                        <tr class="prop odd">
                            <td colspan="3" class="left">
                                ...
                                <div class="fade">${g.message(code: 'dashboard.outgoing.label', args: [session.warehouse.name, startDate, endDate]) }</div>
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'outgoing','status':pending]">
									${g.message(code: 'dashboard.outgoing.pending.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'outgoing','status':shipped]">
									${g.message(code: 'dashboard.outgoing.shipped.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
							<td style="text-align: right;">
								<g:link controller="shipment" action="list" params="['type':'outgoing','status':shipped]">
						...
						<tr>
							...
							<td>
								<g:link controller="shipment" action="list" params="['type':'outgoing','status':received]">
									${g.message(code: 'dashboard.outgoing.received.label', args: [session.warehouse.name]) }
								</g:link>
							</td>
							<td style="text-align: right;">
								<g:link controller="shipment" action="list" params="['type':'outgoing','status':received]">
