<tr class="hasRelated ${counter%2==0?'even':'odd' } ${cssClass}" id="productGroup-${inventoryItem?.productGroup?.id }">
				...
				<div class="action-menu-item">
					<div class="action-menu-item">
						<g:link controller="productGroup" action="edit" id="${inventoryItem?.productGroup?.id }">
							<img src="${resource(dir:'images/icons/silk',file:'pencil.png')}" alt="${g.message(code: 'default.edit.label') }" class="middle"/>
							&nbsp;<g:message code="productGroup.edit.label" />
						</g:link>
					</div>
				</div>
			</div>
	...
	<td class="middle">
		<span class="fade">
			<g:message code="default.various.label"/>
		</span>
	</td>
	<td class="middle">
		<span id="${inventoryItem?.productGroup?.id }" class="expandable">
			${inventoryItem?.productGroup?.name } (${inventoryItem?.productGroup?.products?.size() } ${g.message(code:'products.label') })
		</span>
	...
	<td class="checkable middle center" style="width: 7%; border-left: 1px solid lightgrey;">
		...
		<g:else>
			<span class="fade"><g:message code="default.na.label"/></span>
		</g:else>
	</td>
