<div class="box">
    <h2><g:message code="inventory.incomingTransfer.label"/></h2>
	...
	<g:form action="saveCreditTransaction">
		...
		<table>
			<tr class="prop">
				<td class="name">
					<label><g:message code="transaction.type.label"/></label>
				</td>
				<td class="value">
			...
			<tr class="prop">
				<td class="name">
					<label><g:message code="transaction.date.label"/></label>
				</td>
				<td class="value">
			...
			<tr class="prop">
				<td class="name">
					<label><g:message code="transaction.comment.label"/></label>
				</td>
				<td class="value">
					<span class="comment">
										...
										<tr class="prop row">
											...
											<td>
													...
													<select class="inventoryItem" name="transactionEntries[${i }].inventoryItem.id">
														...
														<g:each var="inventoryItem" in="${command?.productInventoryItems[transactionEntry?.product]?.sort { it.expirationDate } }">
															...
															<option value="${inventoryItem?.id }" ${selected?'selected=selected':'' }>
																${inventoryItem?.lotNumber ?: g.message(code: 'inventory.noLotNumber.message') }
														...
														<option value="0">${g.message(code: 'inventory.enterLotOrSerialNumber.label') }</option>
													</select>
												</span>
												...
												<span class="lotNumberEditor" style="${!displayLotNumberEditor?'display:none;':'' }">
													...
													<img class="undo middle" src="${resource(dir:'images/icons/silk',file:'decline.png')}" title="${g.message(code: 'cancel.label') }" alt="${g.message(code: 'cancel.label') }"/>
												</span>

												<span class="lotNumberReadonly" style="${!displayLotNumberReadonly?'display:none;':'' }">
													<g:if test="${transactionEntry?.inventoryItem }">
														<span class="lotNumber">${transactionEntry?.inventoryItem?.lotNumber ?: g.message(code: 'inventory.noLotNumber.message')}</span>
													...
													<g:else>
														<span class="lotNumber">${transactionEntry?.lotNumber ?: g.message(code: 'inventory.noLotNumber.message')}</span>
													...
													<img class="undo middle" src="${resource(dir:'images/icons/silk',file:'pencil.png')}" title="${g.message(code: 'cancel.label') }" alt="${g.message(code: 'undo.label') }"/>
												</span>
											</td>
											<td>
											...
											<td>
												<img class="add middle" src="${resource(dir:'images/icons/silk',file:'add.png')}" alt="${g.message(code: 'add.label') }"/>
												<img class="delete middle" src="${resource(dir:'images/icons/silk',file:'delete.png')}" alt="${g.message(code: 'delete.label') }"/>
											</td>
											<td>
										...
										<tr class="prop row">
                                            ...
                                            <td>
												...
												<span class="lotNumberEditor" style="${!displayLotNumberEditor?'display:none;':'' }">
													...
													<img class="undo middle" src="${resource(dir:'images/icons/silk',file:'decline.png')}" alt="${g.message(code: 'cancel.label') }"/>
												</span>
												...
												<span class="lotNumberReadonly" style="${!displayLotNumberReadonly?'display:none;':'' }">
													...
													<img class="undo middle" src="${resource(dir:'images/icons/silk',file:'pencil.png')}" alt="${g.message(code: 'undo.label') }"/>
												</span>
											</td>
											<td>
...
<script>
	$(document).ready(function() {
		 ...
		 */
		$(".lotNumber").watermark('${g.message(code:'transaction.addNewLotSerialNumber.label')}');
		...
		$(".inventoryItem").livequery('change', function() {
			...
			if (value == "0") {
				...
				$.getJSON('${request.contextPath}/json/getInventoryItem/' + value, function(data) {
					...
					var errorMessage = "${g.message(code: 'inventory.noLotNumber.message')}"
					...
					if (expMonth && expYear) {
						readonlySpan.find(".expirationDate").text("(${g.message(code: 'default.expires.label')} " + expMonth + " " + expYear + ")");
					}
				});
