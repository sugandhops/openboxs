		...
		<div class="dialog">
			<div class="box">
				...
				<table>
					...
					<tr class="prop">
						<td class="name">
							<label><g:message  code="transaction.transactionDate.label"/></label>
						</td>
						<td class="value">
							<%--<g:datePicker name="transactionDate" value="${commandInstance?.transactionDate}" precision="minute" noSelection="['':'']"/>--%>

							<input type="date" name="transactionDate" value="${commandInstance?.transactionDate}" />
						</td>
					</tr>
					<tr class="prop">
						<td class="name">
							<label><g:message code="default.comments.label"/></label>
						</td>
						<td class="value">
			...
			<div class="box">
				<h2><g:message code="default.lineItems.label" default="Line Items"/></h2>
				<table id="inventoryItemsTable">
					...
					<tr>
						...
						<th class="center"><g:message code="inventory.oldQty.label"/></th>
						<th class="center"><g:message code="inventory.newQty.label"/></th>
						<th class="center"><g:message code="default.comments.label"/></th>
                                ...
                                <tr class="${styleClass} ${status%2==0?'odd':'even'}" style="${recordInventoryRow.error ? 'background-color: #ffcccb;' : ''}">
									...
									<g:else>
											...
											<g:else>
												<g:if test="${!recordInventoryRow?.binLocation?.parentLocation?.hasBinLocationSupport()}">
													<g:message code="default.notSupported.label"/>
												</g:if>
												<g:else>
                                    ...
                                    <td>
										...
										<g:else>
											...
											<g:else>
												<div class="fade">${g.message(code: 'default.never.label')}</div>
											</g:else>
										</g:else>
                                    </td>
                            ...
                            <tr id="emptyRow">
                                <td colspan="5" class="center">
                                    <div class="fade empty center">
                                        <g:message code="inventory.addNewInventoryItem.message"/>
					...
					<tr>
						<td colspan="7">
							<div class="left">
								<button id="addRow" class="button icon add" type="button">
									<g:message code="inventory.addInventoryItem.label"/>
								</button>
							</div>
						...
						<td colspan="7">
                                <div>
                                    ...
                                    <g:link controller="inventoryItem" action="showStockCard"
                                        ...
                                        <g:message code="default.button.discard.label" default="Discard"/>
                                    </g:link>
...
<script>
	...
	<g:each var="row" in="${commandInstance?.recordInventoryRows?.findAll { it?.oldQuantity != 0 || it?.newQuantity != 0}}" status="status">
		var existingInventoryItem = {
				...
				ExpirationDate: '${row?.expirationDate?:g.message(code: 'default.never.label')}',
				Qty: 0
			};
	...
	function getIndex() {
	    alert($.inArray( this.data, inventory.InventoryItems ));
			...
			$(this).autocomplete( {
				...
				select: function(event, ui) {
					...
					if (ui.item.expirationDate) {
						...
						expirationDate.val('${g.message(code: 'default.never.label')}');
					}
				}
			});
		});
...
<tr id="row-0" class="odd">

	<td style="max-width: 200px;">
		<g:selectBinLocationWithOptGroup id="binLocation-0" class="binLocation" name="recordInventoryRows[0].binLocation.id" value="{{= BinLocation}}" noSelection="['':'']"/> 

	</td>

	<td>
		<g:textField id="lotNumber-0" class="lotNumber text" name="recordInventoryRows[1].lotNumber" value="{{= LotNumber}}" size="25" />
	...
	<td class="nowrap">
       <%-- <style>
        ...
        <g:set var="currentYear" value="${java.time.Year.now().value}"/>
		<g:set var="minimumYear" value="${ConfigHelper.minimumExpirationDate[Calendar.YEAR]}"/>
        <g:datePicker name="recordInventoryRows[0].expirationDate"
                      default="none"
					  noSelection="['': '']"
					  years="${minimumYear..currentYear + 20}"
                      precision="day"/>--%>
        <input type="date" name="recordInventoryRows[0].expirationDate"
        />              

    </td>
	<td style="text-align: center; vertical-align: middle;">
		{{= Qty}}
		<g:hiddenField id="oldQuantity-0" class="oldQuantity"
			name="recordInventoryRows[0].oldQuantity" value="{{= Qty}}"/>
	</td>
	<td style="text-align: center; vertical-align: middle;">
		<g:textField
        id="newQuantity-0" type="number" class="newQuantity text"
        name="recordInventoryRows[0].newQuantity"
        size="8" value="{{= Qty}}" />
	</td>
	<td class="center middle">
	    <g:textField id="comment-0" class="text"
                     placeholder="${g.message(code:'transactionEntry.comment.message')}"
                     name="recordInventoryRows[0].comment" style="width:100%;" value="" />
	</td>
	<td class="center">
		<button onclick="removeRow(0);" class="button icon trash" tabIndex="-1">
			${g.message(code: 'default.button.delete.label')}&nbsp;
		</button>
		<g:hiddenField name="recordInventoryRows[0].id" value="{{= Id}}" size="1" />
	</td>
