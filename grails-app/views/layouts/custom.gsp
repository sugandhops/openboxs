...
<head>
    ...
    <script src="${createLinkTo(dir:'js/jquery/', file:'jquery-3.6.0.js')}"  type="text/javascript" ></script>
    ...
    <g:if test="${ session?.user?.locale && session?.user?.locale != 'en' && session?.user?.locale == 1}">
        <script src="${createLinkTo(dir:'js/jquery.validation/', file:'messages_'+ session?.user?.locale + '.js')}"  type="text/javascript" ></script>
    </g:if>
    <g:else>
        <script src="${createLinkTo(dir:'js/jquery.validation/', file:'messages_en.js')}"  type="text/javascript" ></script>
    </g:else>
...
<div id="doc3">
    ...
    <g:if test="${grailsApplication.config.system.notification.enabled}">
        <div class="notice">
            ${grailsApplication.config.system.notification.message}
        </div>
    </g:if>
    ...
    <g:if test="${session.useDebugLocale}">
        ...
        <div id="debug-header" class="notice" style="margin: 10px;">
            <g:message code="localization.custom.message"/>
            <g:link controller="localization" action="list" class="button">
                <g:message code="default.list.label" args="[message(code: 'localizations.label')]"/>
            </g:link>
            <g:link controller="localization" action="create" class="button">
                <g:message code="default.add.label" args="[message(code: 'localization.label')]"/>
            </g:link>
            <div class="right">
                <g:link controller="user" action="disableLocalizationMode" class="button">
                    <g:message code="localization.disable.label"/>
                </g:link>
            </div>
               ...
               <%-- <g:displayLogo location="${session?.warehouse?.id}" includeLink="${true}" /> --%>
                ...
                <div class="tooltip2">
                    ...
                    >
                        ...
                        <g:if  test="${grailsApplication.config.logo.label}">
                            <span class="location-chooser__button-tag">
                                ${grailsApplication.config.logo.label}
                            </span>
                        </g:if>
                    </button>
...
<g:if test="${session.user && Boolean.valueOf(grailsApplication.config.jira.issue.collector.enabled)}">
    <script type="text/javascript" src="${grailsApplication.config.jira.issue.collector.url}"></script>
</g:if>
...
<g:javascript>
    ...
    $(document).ready(function() {
        ...
        $(".btn-show-dialog").on("click", function (event) {
            event.preventDefault();
...
<g:if test="${session.user && Boolean.valueOf(grailsApplication.config.scannerDetection.enabled)}">
...
<g:if test="${session.user && Boolean.valueOf(grailsApplication.config.uservoice.widget.enabled)}">
    <script type="text/javascript">
        ...
        UserVoice.push(['addTrigger', {
            mode: 'contact',
            trigger_style: 'tab',
            trigger_position: '${grailsApplication.config.uservoice.widget.position?:"bottom-right"}',
            trigger_background_color: '#448dd6',
            locale: '${session?.user?.locale?:"en"}'
        }]);
    </script>
</g:if>
<g:if test="${grailsApplication.config.zopim.widget.enabled}">
    <!--Start of Zopim Live Chat Script-->
    <script type="text/javascript">
        window.$zopim||(function(d,s){var z=$zopim=function(c){z._.push(c)},$=z.s=
                d.createElement(s),e=d.getElementsByTagName(s)[0];z.set=function(o){z.set.
                _.push(o)};z._=[];z.set._=[];$.async=!0;$.setAttribute("charset","utf-8");
            $.src="${grailsApplication.config.zopim.widget.url}";z.t=+new Date;$.
