...
<html>
	<head>
        ...
        <g:set var="entityName" value="${g.message(code: 'product.label', default: 'Product')}" />

        <g:if test="${productInstance?.id}">
	        <title>${productInstance?.productCode } ${productInstance?.name }</title>
		</g:if>
		<g:else>
	        <title><g:message code="product.add.label" /></title>
			<content tag="label1"><g:message code="inventory.label"/></content>
		</g:else>
                    ...
                    <g:if test="${productInstance?.id }">
                        ...
                        <li>
                            <a href="${request.contextPath}/product/renderTemplate/${productInstance?.id}?templateName=productCatalogs">
                                <g:message code="product.catalogs.label" default="Catalogs"/>
                            </a>
                        </li>
                        <li>
                            <a href="${request.contextPath}/product/renderTemplate/${productInstance?.id}?templateName=productGroups">
                                <g:message code="product.productGroups.label" default="Product Groups"/>
                            </a>
                        </li>
                        <li>
	    	...
	    	$(document).ready(function() {
              ...
              $('#productType').livequery('change', function() {
                ...
                if (response && response.fieldRemoved) {
                  var success = confirm(
                    '${g.message(code: 'product.productType.confirmChange.message', default: 'Changing the product type will delete data you have entered for this product. Would you like to proceed?')}');
                  if (success) {
                    prevProdType = $(this)
                    .val();
                  } else {
