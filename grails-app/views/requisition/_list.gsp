	...
	<thead>
		<tr>
			...
			<g:sortableColumn property="description" params="${pageParams}"
				title="${g.message(code: 'default.description.label', default: 'Description')}" />

            <g:sortableColumn property="origin" params="${pageParams}"
                              title="${g.message(code: 'requisition.origin.label', default: 'Origin')}" />

            <g:sortableColumn property="requestedBy" params="${pageParams}"
                              title="${g.message(code: 'requisition.requested.label', default: 'Requested')}" />
            <th>
                <g:message code="default.created.label"/>
            </th>
            <th>
                <g:message code="default.updated.label"/>
            </th>
		</tr>
	</thead>
	<tbody>
		...
		<g:each in="${requisitions}" status="i" var="requisition">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td>${requisition.requestedBy?:g.message(code:'default.none.label')}</td>
                <td>${requisition.createdBy?:g.message(code:'default.none.label')}</td>
                <td>${requisition.updatedBy?:g.message(code:'default.none.label')}</td>
			</tr>
		</g:each>
	</tbody>
</table>
