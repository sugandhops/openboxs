...
<g:if test="${requisition?.id}">
    ...
    <div id="flow-header" class="buttonBar">
                    ...
                    <div class="${styleClass}">
                        <g:if test="${requisition?.id}">
                            <g:link controller="requisition" action="${wizardStep?.value}" event="${wizardStep?.value}" id="${requisition?.id}">
                                <span>${status+1}</span>
                                <g:message code="requisition.wizard.${wizardStep?.value}.label" default="${wizardStep?.value}"/>
                            </g:link>
                        </g:if>
                        <g:else>
                            <a href="#">
        ...
        <g:if test="${!(requisition?.origin == session?.warehouse || requisition?.destination == session?.warehouse)}">
            <div class="error">
                <g:message code="requisition.wrongLocation.message" default="CAUTION: You appear to be logged into the wrong location! Making any changes to this requisition within this location may cause it to become invalid."/>
            </div>
        </g:if>
    </div>
</g:if>