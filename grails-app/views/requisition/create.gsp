...
<g:set var="entityName" value="${g.message(code: 'requisition.label', default: 'Requisition')}" />
<title><g:message code="${requisition?.id ? 'default.edit.label' : 'default.create.label'}" args="[entityName]" /></title>
...
<body>
		...
		<div class="yui-u first">
			...
			<g:form name="requisitionForm" method="post" action="save" useToken="true">
										...
										<td class="value">
											<g:if test="${params?.editHeader || !requisition?.id }">
												<input name="requestedById"
													...
													placeholder="${g.message(code:'requisition.requestedBy.label')}"
						...
						<div class="yui-u">
							...
							<table class="">
								<tbody>
									...
									<tr class="prop">
										<td class="name"><label><g:message code="requisition.requestedDeliveryDate.label" /></label></td>
										<td class="value">
											<g:if test="${params?.editHeader || !requisition?.id }">
												<input
										...
										<td class="value">
											...
											<g:if test="${params?.editHeader || !requisition?.id }">
												<g:textArea name="description" cols="80" rows="5"
													placeholder="${g.message(code:'requisition.description.label')}"
							...
							<thead>
								<tr class="prop odd">
									<th class="list-header">
										${g.message(code: 'requisitionItems.label')}
									</th>
									<th class="list-header center">
										${g.message(code: 'requisitionItem.quantity.label')}
									</th>
									<th class="list-header center">
										${g.message(code: 'product.uom.label', default: 'UOM')}
									</th>
									<th class="center">
										${g.message(code: 'requisitionItem.delete.label')}
									</th>
								</tr>
							<thead>
							<tbody data-bind="foreach: requisition.requisitionItems">
								<tr class="requisitionItemsRow">
									<td class="list-header"><input type="hidden"
										...
										placeholder="${g.message(code:'requisition.addItem.label')}"
				...
				<div class="buttons">
					...
					<g:if test="${requisition?.id && params.editHeader}">
						<button class="button" name="saveAndContinue">${g.message(code:'default.button.saveAndContinue.label', default: 'Save & Continue') }</button>
					...
					<g:else>
							...
							<g:if test="${requisition?.id && !params.editHeader}">
								<button class="button"
									...
									data-bind='click: requisition.addItem'>
										${g.message(code:'requisition.addNewItem.label')}</button>
							</g:if>
						</div>
						...
						<div class="right">
							...
							<button id="save-requisition" class="button">
								${g.message(code: 'default.button.next.label')}</button>
						</div>
					</g:else>
				</div>
			</g:form>
    ...
    $(function () {
        ...
        $("#requisitionForm").validate({
            ...
            messages: {
                product: { required: "${g.message(code: 'inventoryItem.productNotSupported.message')}" },
                requestedBy: { required: "${g.message(code: 'person.notFound.message')}" }
            }
        });

        if (!viewModel.requisition.name())
            viewModel.requisition.name("" + requisitionTypes[viewModel.requisition.type()] + " " + "${g.message(code: 'requisition.label')}");
        ...
        var updateDescription = function () {
            ...
            var description = type + "${g.message(code: 'requisition.label', default: 'Requisition')}";
            if(depot) {
                description += " - " + depot;
            }
            if(program != "") {
