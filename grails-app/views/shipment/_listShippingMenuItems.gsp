<div class="action-menu-item">
	...
	<img src="${resource(dir:'images/icons/silk',file:'zoom.png')}"
		alt="Show Shipment" style="vertical-align: middle" />&nbsp;<g:message code="shipping.showDetails.label"/></g:link>
</div>
	...
	<g:if test="${(session?.warehouse?.id == shipmentInstance?.origin?.id) || (!shipmentInstance?.origin?.isWarehouse() && session?.warehouse?.id == shipmentInstance?.destination?.id)}">
		<div class="action-menu-item">
			...
			<img src="${resource(dir:'images/icons/silk',file:'pencil.png')}"
				alt="Edit shipment" style="vertical-align: middle" />&nbsp;<g:message code="shipping.editShipment.label"/></g:link>
		</div>
	</g:if>
	<g:if test="${(session?.warehouse?.id == shipmentInstance?.origin?.id) || (!shipmentInstance?.origin?.isWarehouse() && session?.warehouse?.id == shipmentInstance?.destination?.id)}">
		<div class="action-menu-item">
			<g:link controller="createShipmentWorkflow" action="createShipment" event="enterTrackingDetails" id="${shipmentInstance?.id }" params="[skipTo:'Tracking']">
				<img src="${resource(dir:'images/icons/silk',file:'map.png')}"
					class="middle"/>&nbsp;<g:message code="shipping.enterTrackingDetails.label"/>
			</g:link>
		</div>
	</g:if>
	<g:if test="${(session?.warehouse?.id == shipmentInstance?.origin?.id) || (!shipmentInstance?.origin?.isWarehouse() && session?.warehouse?.id == shipmentInstance?.destination?.id)}">
		<div class="action-menu-item">
			<g:link controller="createShipmentWorkflow" action="createShipment" event="enterContainerDetails" id="${shipmentInstance?.id }" params="[skipTo:'Packing']">
				<img src="${resource(dir:'images/icons/silk',file:'package.png')}"
					class="middle"/>&nbsp;<g:message code="shipping.editPackingList.label"/>
			</g:link>
		</div>
	</g:if>
	...
	<div class="action-menu-item">
		<g:link controller="document" action="download" id="${document.id}">
			...
			<g:if test="${document?.filename}">
				<g:message code="document.download.label"/> ${document?.filename } <span class="fade"><format:metadata obj="${document?.documentType}"/></span>
			</g:if>
			<g:else>
				<g:message code="document.download.label"/> <format:metadata obj="${document?.documentType?.name}"/>
			</g:else>
		</g:link>
	</div>
</g:each>
<div class="action-menu-item">
	<g:link controller="doc4j" action="downloadPackingList" id="${shipmentInstance?.id }">
		<img src="${resource(dir:'images/icons/silk',file:'page_white_excel.png')}"
			alt="View Packing List" style="vertical-align: middle"/>&nbsp;<g:message code="shipping.downloadPackingList.label"/> (.xls)
	</g:link>
</div>
<g:if test="${session?.warehouse?.id == shipmentInstance?.origin?.id ||
	...
	<div class="action-menu-item">
		...
		alt="Delete Shipment" style="vertical-align: middle" />&nbsp;
			<g:message code="shipping.deleteShipment.label"/>
		</g:link>
	</div>
</g:if>
